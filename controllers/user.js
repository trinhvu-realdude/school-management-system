...
router.get("/user_list", function (req, res) {
    (async function () {
        ...
        result.forEach(function (user) {
            ...
            let strRegTime = regDate.getHours() + ":" + regDate.getMinutes() + ", " + regDate.getDate() + "/" + (
                regDate.getMonth() + 1
            ) + "/" + regDate.getFullYear()
            let adminAction = "<td><a href=\"/user_edit_" + user["_id"] + "\">Edit</a></td>" + "<td><a href=\"javascript:confirmDelete('" + user["_id"] + "')\">Delete</a></td>"
            if (role["role"] != 9) 
                adminAction = ""
            
            tbtext = tbtext + "<tr><th scope=\"row\">" + stt + "</th>" + "<td>" + uType + "</td>" + "<td>" + user["username"] + "</td>" + "<td>" + user["email"] + "</td>" + "<td>" + strRegTime + "</td>" + adminAction + "</tr>"
            stt++
        })
        let parts = {
            tb: tbtext
